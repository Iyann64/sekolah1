    ...
    <div class="hero-content">
        ...
        <div class="hero-btns">
        <a href="<?= base_url('tentang') ?>" class="btn-hero-p">🏫 Jelajahi Sekolah</a>
        <a href="<?= base_url('ppdb') ?>"    class="btn-hero-s">✏️ Daftar PPDB 2026</a>
        </div>
    </div>
    ...
    <div class="prog-grid">
        ...
        <div class="prog-card reveal d1" data-cat="ekskul"><div class="pc-top pct5">⚽</div><div class="pc-body"><span class="pc-tag" style="background:#EDE7F6;color:#6A1B9A">Ekskul</span><div class="pc-title">Olahraga & Paskibra</div><div class="pc-desc">Futsal, bulu tangkis, renang, dan Paskibra untuk jiwa sportif.</div></div></div>
    ...
    <div class="berita-layout">
        ...
        <a href="<?= base_url('berita/'.$f['slug']) ?>" class="news-featured reveal" style="text-decoration:none">
            ...
            <p class="nf-excerpt"><?= esc(character_limiter(strip_tags($f['isi'] ?? ''), 150)) ?></p>
            <div class="nf-meta">📅 <?= date('d F Y', strtotime($f['tanggal'])) ?> &nbsp;·&nbsp; 👁 <?= number_format($f['views']) ?> dibaca</div>
            </div>
        </a>
        ...
        <div class="sidebar-widget">
            ...
            <div class="ag-item">
                <div class="ag-date" style="background:<?= $agColors[$i % 5] ?>">
                <div class="ag-day"><?= date('d', strtotime($ag['tanggal'])) ?></div>
                <div class="ag-mon"><?= date('M', strtotime($ag['tanggal'])) ?></div>
                </div>
                <div>
                <div class="ag-title"><?= esc($ag['judul']) ?></div>
                <div class="ag-desc"><?= esc($ag['tempat'] ?? '-') ?><?= $ag['waktu'] ? ' · '.date('H.i', strtotime($ag['waktu'])).' WIB' : '' ?></div>
                </div>
            </div>
            <?php endforeach; else: ?>
            <!-- Fallback statis -->
    ...
    <div class="gal-filter reveal">
        ...
        <button class="gf" onclick="filterGal(this,'Olahraga')">Olahraga</button>
        <button class="gf" onclick="filterGal(this,'Lingkungan')">Lingkungan</button>
    ...
    <div class="ppdb-inner">
        ...
        <a href="<?= base_url('ppdb') ?>" class="btn-ppdb">✏️ Daftar / Tanya Info PPDB →</a>
    </div>
    </section>
    ...
    <div class="kontak-layout">
        ...
        <div style="background:#FFEBEE;border-left:4px solid #EF5350;border-radius:10px;padding:12px 16px;font-size:13px;color:#C62828;margin-bottom:16px">
            ⚠️ <?= session()->getFlashdata('error') ?>
        </div>
        <?php endif; ?>
        <form id="formWrap" action="<?= base_url('kontak/kirim') ?>" method="POST">
            <?= csrf_field() ?>
            <div class="form-row">
            <div class="fg"><label>Nama Lengkap *</label><input type="text"  name="nama"    placeholder="Nama Anda"      value="<?= old('nama') ?>"    required></div>
            <div class="fg"><label>No. Telepon</label>   <input type="tel"   name="telepon" placeholder="08xx-xxxx-xxxx" value="<?= old('telepon') ?>"></div>
            </div>
            <div class="fg"><label>Email *</label><input type="email" name="email" placeholder="email@anda.com" value="<?= old('email') ?>" required></div>
            <div class="fg">
            ...
            <div class="fg"><label>Pesan *</label><textarea name="pesan" placeholder="Tulis pesan Anda di sini..." required><?= old('pesan') ?></textarea></div>
